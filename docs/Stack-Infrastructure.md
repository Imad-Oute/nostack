...
Every external application (Flowise, Langflow, OpenWebUI, Zep, etc.) gets its own folder. This allows:
*   Full customization
*   Version tracking
*   Local development
*   Hot-reloading
...
**Result:** Your entire stack becomes portable → copy/paste the folder anywhere, and it works.
...
Instead of configs being spread across `/etc/`, inside containers, Docker volumes, or hardcoded image defaults, they are centralized here.
...
1.  Add one folder under `/services`
2.  Add one folder under `/data` (if needed)
3.  Add one section to `docker-compose.yml`
4.  Add config file under `/config`
